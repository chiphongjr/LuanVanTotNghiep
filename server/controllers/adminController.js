...
export const deleteUser = catchAsyncErrors(async (req, res, next) => {
  ...
  const orderItemCheck = await database.query(
    `Select * from orders where buyer_id = $1`,
    [id]
  );

  if (orderItemCheck.rows[0])
    return next(
      new ErrorHandler("Không thể xóa người dùng vì đơn hàng còn tồn tại", 400)
    );
...
export const updateUserStatus = catchAsyncErrors(async (req, res, next) => {
  ...
  const allowStatus = ["active", "blocked"];
  ...
  res.status(200).json({
    success: true,
    message: status === "blocked" ? "Đã khóa tài khoản " : "Đã mở tài khoản",
    user: result.rows[0],
  });
});
...
export const dashboardStats = catchAsyncErrors(async (req, res, next) => {
  ...
  const topSellingProductsQuery = await database.query(`
    ...
    WHERE o.paid_at IS NOT NULL and o.order_status ='Delivered'
    GROUP BY p.name, p.images, c.name, p.ratings
