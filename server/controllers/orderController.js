...
export const placeNewOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  if (payment_type === "Online" && final_price < 20000) {
    return next(
      new ErrorHandler("Đơn hàng dưới 20.000 đ, chọn COD để đặt hàng", 400)
    );
  }
...
GROUP BY o.id, s.id, p.payment_type
        `);
